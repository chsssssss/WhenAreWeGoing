...
- [x] 앱 아이콘 교체 — 오렌지 배경(`#E2693D`) + 오프화이트(`#FAF7F2`) 위치핀 안에 북마크 리본이 뚫린 모양으로 교체. 어댑티브 아이콘 배경은 단색 벡터(`drawable/ic_launcher_background.xml`)로 남기고, 포그라운드는 제공받은 원본 PNG에서 배경/글리프 색을 분리해 밀도별(`drawable-mdpi`~`xxxhdpi`) 래스터로 생성 — 북마크 리본 부분은 투명 처리해서 배경색이 그대로 비쳐 보이게 함(원본 디자인과 동일). 레거시 `mipmap-*/ic_launcher(.round).webp`도 같은 방식으로 만든 PNG로 교체(정사각형은 풀블리드, 원형은 원형 마스크 적용). 글리프가 어댑티브 아이콘 세이프존(66/108) 안에 들어오는 것 확인
...
- [ ] **카카오 디벨로퍼스 콘솔에 새 릴리스 키 해시 등록 필요** — 위 실기기 테스트 중 지도가 완전히 빈 화면으로 뜨는 걸 발견했는데, R8 문제가 아니라 `MapAuthException(401): Unauthorized`였다. 이 릴리스 업로드 키스토어는 방금 새로 만든 거라 카카오 콘솔의 Android 플랫폼 키 해시 목록에 아직 없어서 생기는, 당연하고 별개인 문제. 키 해시는 `keytool -exportcert -alias eonje-upload -keystore ~/.android/eonje-upload-keystore.jks | openssl sha1 -binary | openssl base64` → `Hax1qjOPQX3hNGF6cVKAUh5eLPA=` (로그의 `KA=[... origin/Hax1qjOPQX3hNGF6cVKAUh5eLPA= ...]`와 일치 확인됨). [카카오 디벨로퍼스](https://developers.kakao.com) > 내 애플리케이션 > 플랫폼 > Android에 이 값을 등록할 것 — 이미 등록해둔 디버그 키 해시는 그대로 두고 추가하면 됨. **주의**: Play Console에 업로드하면 Play App Signing이 이 업로드 키와 별개인 자체 배포 서명 키로 앱을 다시 서명하므로, 실제 사용자에게 배포되는 빌드가 지도를 쓰려면 Play Console의 "앱 무결성(App integrity)" 메뉴에서 그 배포 서명 키의 SHA-1도 확인해서 카카오 콘솔에 추가로 등록해야 함(최초 업로드 후에만 확인 가능)
